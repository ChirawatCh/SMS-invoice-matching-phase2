## CP Ram at 18/09/2023 -> First Draft
...
Base_PATH = 'CPRam Invoice.xlsx'
# B2B_PATH = 'B2B.csv'
CPFM_PATH = 'CPFM.csv'

df_Base = pd.read_excel(Base_PATH,converters={'Sold-to':str,'Purchase Order No.':str,'Bill.Doc':str,'Doc Date':str}, skiprows=3)
...
df_Base = df_Base.rename(columns={'Purchase Order No.': 'PO'})
df_Base = df_Base.rename(columns={'Bill.Doc': 'Invoice_No'})
# df_Base = df_Base.rename(columns={'Store No.': 'Store_No'}) Not Have
df_Base = df_Base.rename(columns={'Account Memo': 'Store_Name'})
df_Base = df_Base.rename(columns={'Doc Date': 'Invoice_Date'})
df_Base = df_Base.rename(columns={'    Val THB': 'Exc_Vat'})
df_Base = df_Base.rename(columns={'   Tax Val': 'Tax'}) 
df_Base = df_Base.rename(columns={'Amt in Loc.curr': 'Total_Amt'})
df_Base['Store_No'] = ""
# df_Base['Tax'] = 0
# df_Base['PO'] = ""
...
## Write the reconciled data_v1 and the counts to a single Excel file ##
with pd.ExcelWriter("reconciled_data_CPRam.xlsx") as writer:
...
df_CPFM_diff_select.to_csv('cpfm_b2b_diff_CPRam.csv', index=False)
