## Perfect Companion at 18/09/2023 -> First Draft
...
Base_PATH = 'PerfectCompanion Invoice.xlsx'
# B2B_PATH = 'B2B.csv'
CPFM_PATH = 'CPFM.csv'

df_Base = pd.read_excel(Base_PATH,converters={'เลขที่ PO':str,'เลขที่ใบแจ้งหนี้':str,'สาขาที่ออกใบกำกับภาษี':str,'วันที่ใบแจ้งหนี้':str}, skiprows=6)
...
df_Base = df_Base.rename(columns={'เลขที่ PO': 'PO'})
df_Base = df_Base.rename(columns={'เลขที่ใบแจ้งหนี้': 'Invoice_No'})
# df_Base = df_Base.rename(columns={'Store No.': 'Store_No'}) Not Have
df_Base = df_Base.rename(columns={'สถานที่ส่งสินค้า': 'Store_Name'})
df_Base = df_Base.rename(columns={'วันที่ใบแจ้งหนี้': 'Invoice_Date'})
df_Base = df_Base.rename(columns={'จำนวนเงิน(ก่อนVAT)': 'Exc_Vat'})
df_Base = df_Base.rename(columns={'VAT 7%': 'Tax'}) 
df_Base = df_Base.rename(columns={'จำนวนเงิน(รวมVAT)': 'Total_Amt'})
df_Base['Store_No'] = ""
# df_Base['Tax'] = 0
# df_Base['PO'] = ""
...
df_Base.drop(df_Base.tail(1).index,inplace=True)
...
df_merge['Exc_Vat_difference'] = df_merge['Exc_Vat'] - df_merge['rSumNett']
df_merge['Tax_difference'] = df_merge['Tax_BASE'] - df_merge['Tax_CPFM']
df_merge['Total_Amt_difference'] = df_merge['Total_Amt_BASE'] - df_merge['Total_Amt_CPFM']
...
## Create dataframes only different values ##
df_CPFM_diffrNett = df_merge[(df_merge['Total_Amt_difference'].notnull()) & (df_merge['Total_Amt_difference'] != 0) & (df_merge['null_report'] == '')]
df_CPFM_diffrTax = df_merge[(df_merge['Tax_difference'].notnull()) & (df_merge['Tax_difference'] != 0) & (df_merge['null_report'] == '')]
...
## Write the reconciled data_v1 and the counts to a single Excel file ##
with pd.ExcelWriter("reconciled_data_PerfectCompanion.xlsx") as writer:
...
df_CPFM_diff_select.to_csv('cpfm_b2b_diff_PerfectCompanion.csv', index=False)
