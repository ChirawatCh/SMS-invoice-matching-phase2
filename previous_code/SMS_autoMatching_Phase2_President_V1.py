...
## Merge the dataframes with suffixes added to duplicate column names ##
df_merge = pd.merge(df_Base, df_CPFM, on=['Invoice_No'], how='outer', suffixes=('_BASE', '_CPFM'))
df_merge['Exc_Vat_difference'] = df_merge['Exc_Vat'] - df_merge['rSumNett']
...
## Write the reconciled data_v1 and the counts to a single Excel file ##
with pd.ExcelWriter("reconciled_data_President.xlsx") as writer:
...
df_CPFM_diff_select.to_csv('cpfm_b2b_diff_President.csv', index=False)
